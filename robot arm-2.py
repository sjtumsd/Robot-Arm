# -*- coding: utf-8 -*-
"""
Created on Mon Aug 14 16:58:12 2023
...
base_link=chrono.ChBodyEasyMesh("4dofArm/base_link.obj",10000)
...
link1=chrono.ChBodyEasyMesh("4dofArm/link1.obj",10000)
...
link3.GetVisualShape(0).SetColor(chrono.ChColor(0.1, 0.1, 0.1))
sys.Add(link3)
...
link4.GetVisualShape(0).SetColor(chrono.ChColor(0.1, 0.1, 0.1))
sys.Add(link4)
...
vis.AddSkyBox()
vis.AddCamera(chrono.ChVectorD(0.4,0.4,0.5), chrono.ChVectorD(0,0.1,0))
...
while vis.Run():
    ...
    my_angularspeed1 = chrono.ChFunction_Const((theta_now[0]-theta_last[0])/1e-3)
    my_motor1.SetMotorFunction(my_angularspeed1)
    my_angularspeed2 = chrono.ChFunction_Const(-(theta_now[1]-theta_last[1])/1e-3)
    my_motor2.SetMotorFunction(my_angularspeed2)
    my_angularspeed3 = chrono.ChFunction_Const(-(theta_now[2]-theta_last[2])/1e-3)
    my_motor3.SetMotorFunction(my_angularspeed3)
    my_angularspeed4 = chrono.ChFunction_Const(-(theta_now[3]-theta_last[3])/1e-3)
    my_motor4.SetMotorFunction(my_angularspeed4)
    my_angularspeedClaw1 = chrono.ChFunction_Const((theta_now[4]-theta_last[4])/1e-3)
    my_motorClaw1.SetMotorFunction(my_angularspeedClaw1)
    my_angularspeedClaw2 = chrono.ChFunction_Const(-(theta_now[4]-theta_last[4])/1e-3)
    my_motorClaw2.SetMotorFunction(my_angularspeedClaw2)
    for i in range(5):
        theta_last[i]=theta_now[i]
